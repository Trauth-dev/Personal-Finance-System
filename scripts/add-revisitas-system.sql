...
CREATE INDEX IF NOT EXISTS idx_revisitas_user_id ON crm_revisitas(user_id);
CREATE INDEX IF NOT EXISTS idx_revisitas_perfil_id ON crm_revisitas(perfil_id);
CREATE INDEX IF NOT EXISTS idx_revisitas_cliente_id ON crm_revisitas(cliente_id);
CREATE INDEX IF NOT EXISTS idx_revisitas_fecha ON crm_revisitas(fecha_programada);
CREATE INDEX IF NOT EXISTS idx_revisitas_estado ON crm_revisitas(estado);
CREATE INDEX IF NOT EXISTS idx_revisitas_seguimiento ON crm_revisitas(seguimiento_id);

-- Tabla para sugerencias de re-visitas automaticas (pendientes de confirmacion)
CREATE TABLE IF NOT EXISTS crm_revisitas_sugeridas (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  perfil_id UUID REFERENCES perfiles_empresariales(id) ON DELETE CASCADE,
  cliente_id UUID NOT NULL REFERENCES clientes(id) ON DELETE CASCADE,
...
-- Indice para sugerencias pendientes
CREATE INDEX IF NOT EXISTS idx_revisitas_sugeridas_pendientes 
ON crm_revisitas_sugeridas(user_id, estado) WHERE estado = 'pendiente';
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'crm_revisitas' AND policyname = 'Users can manage own revisitas') THEN
    CREATE POLICY "Users can manage own revisitas" ON crm_revisitas
      FOR ALL USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'crm_revisitas_sugeridas' AND policyname = 'Users can manage own sugerencias') THEN
