...
import { DashboardTerciarioClient } from "./page-client"

export const revalidate = 0

export default async function DashboardTerciarioPage({
  searchParams,
}: {
  ...
  if (!perfilActivo) {
    return <div>No se encontró el perfil personal</div>
  }
      ...
      <DashboardTerciarioClient initialMonth={currentMonthValue}>
        <div className="p-3 sm:p-4 md:p-6">
          <div className="mb-4 sm:mb-6 space-y-4 sm:space-y-6">
            ...
            />
            <ComparativoMesAnterior perfilId={perfilActivo.id} fechaInicio={primerDiaMes} fechaFin={ultimoDiaMes} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 rounded-lg bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200">
                      ...
                      <div className="text-3xl font-bold mb-2 text-slate-900">
                        {new Date(selectedYear, selectedMonth + 1, 0).getDate() - diasTranscurridos}
                      </div>
                      <Progress
                        value={(diasTranscurridos / new Date(selectedYear, selectedMonth + 1, 0).getDate()) * 100}
                    ...
                    <div className="p-4 rounded-lg bg-gradient-to-br from-pink-50 to-rose-50 border-2 border-pink-200">
                      ...
                      <div className="text-2xl font-bold mb-2 text-slate-900">
                        {formatGuaranies(promedioDiario * new Date(selectedYear, selectedMonth + 1, 0).getDate())}
                      </div>
                      <Progress
                        value={
                          (totalEgresos / (promedioDiario * new Date(selectedYear, selectedMonth + 1, 0).getDate())) *
                          100
                        }
