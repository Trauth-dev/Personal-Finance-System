  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (tipoDeuda === "prestamo") {
        ...
        deudaData.monto_cuota = formData.monto_cuota ? Number.parseFloat(formData.monto_cuota) : null
      } else {
        deudaData.limite_credito = formData.limite_credito ? Number.parseFloat(formData.limite_credito) : null
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full mt-2 inline-block ${deuda.tipo_deuda === "tarjeta_credito" ? "bg-purple-500/20 text-purple-300" : "bg-blue-500/20 text-blue-300"}`}
                >
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 rounded-lg bg-muted/30">
              ...
              <p className="text-lg font-bold">{formatGuaranies(Number(deuda.monto_total))}</p>
            </div>
          ...
          {deuda.tipo_deuda === "prestamo" && deuda.cuotas_totales && (
            <div className="p-3 rounded-lg bg-muted/30">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <span className="font-bold text-lg">{deuda.cuotas_pagadas}</span>
                  <span className="text-muted-foreground"> / {deuda.cuotas_totales}</span>
                </div>
              </div>
              {deuda.monto_cuota && (
                <p className="text-sm mt-2">
                  Valor cuota: <span className="font-semibold">{formatGuaranies(Number(deuda.monto_cuota))}</span>
                </p>
              )}
            </div>
          )}
          ...
          <div className="flex gap-2 pt-2">
            <Button variant="destructive" size="sm" onClick={() => handleDeleteDeuda(deuda.id)} className="gap-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-red-500/20 to-rose-500/10 border-2 border-red-500/30">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{deudas.length} deuda(s) registrada(s)</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-green-500/20 to-emerald-500/10 border-2 border-green-500/30">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{porcentajePagado.toFixed(1)}% completado</p>
          </CardContent>
        </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="monto_total">
                    {tipoDeuda === "tarjeta_credito" ? "Saldo Actual *" : "Monto Total *"}
                  </Label>
                  <Input
                    id="monto_total"
                    type="number"
